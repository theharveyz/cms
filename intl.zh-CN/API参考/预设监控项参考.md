...
-   基础监控项
    ...
    |VPC\_PublicIP\_InternetInRate|专有网络公网流入带宽|bits/s|instanceId|Average 、Minimum 、Maximum|
    |VPC\_PublicIP\_InternetOutRate|专有网络公网流出带宽|bits/s|instanceId|Average 、Minimum 、Maximum|
...
-   操作系统级别监控项
    ...
    |memory\_totalspace|Host.mem.total，内存总量|bytes|instanceId|Average 、Minimum 、Maximum|
    |memory\_usedspace|Host.mem.used，已用内存量 ，用户程序使用的内存 + buffers + cached，buffers为缓冲区占用的内存空间，cached为系统缓存占用的内存空间|bytes|instanceId|Average 、Minimum 、Maximum|
    |memory\_actualusedspace|Host.mem.actualused，用户实际使用的内存，计算方法为（used - buffers - cached）|bytes|instanceId|Average 、Minimum 、Maximum|
    |memory\_freespace|Host.mem.free，剩余内存量|bytes|instanceId|Average 、Minimum 、Maximum|
    ...
    |diskusage\_used|Host.diskusage.used，磁盘的已用存储空间|bytes|instanceId, device|Average 、Minimum 、Maximum|
    |diskusage\_utilization|Host.disk.utilization，磁盘使用率|%|instanceId, device|Average 、Minimum 、Maximum|
    |diskusage\_free|Host.diskusage.free，磁盘的剩余存储空间|bytes/s|instanceId, device|Average 、Minimum 、Maximum|
    |diskusage\_total|Host.diskussage.total，磁盘存储总量|bytes|instanceId, device|Average 、Minimum 、Maximum|
    |disk\_readbytes|Host.disk.readbytes，磁盘每秒读取的字节数|bytes/s|instanceId, device|Average 、Minimum 、Maximum|
    |disk\_writebytes|Host.disk.writebytes，磁盘每秒写入的字节数|bytes/s|instanceId, device|Average 、Minimum 、Maximum|
    ...
    |networkin\_rate|Host.netin.rate，网卡每秒接收的比特数，即网卡的上行带宽|bits/s|instanceId、device|Average 、Minimum 、Maximum|
    |networkout\_rate|Host.netout.rate，网卡每秒发送的比特数，即网卡的下行带宽|bits/s|instanceId、device|Average 、Minimum 、Maximum|
...
|MySQL\_NetworkInNew|Mysql每秒网络入流量|bits/s|instanceId|Average 、Minimum 、Maximum|
|MySQL\_NetworkOutNew|Mysql每秒网络出流量|bits/s|instanceId|Average 、Minimum 、Maximum|
|SQLServer\_NetworkInNew|SQLServe每秒网络入流量|bits/s|instanceId|Average 、Minimum 、Maximum|
|SQLServer\_NetworkOutNew|SQLServer每秒网络出流量|bits/s|instanceId|Average 、Minimum 、Maximum|
...
参见[OSS监控项参考](https://help.aliyun.com/document_detail/31879.html)。
...
-   新版本
    ...
    |IntranetIn|写入网络带宽|Bytes/s|instanceId|Average 、Minimum 、Maximum|
    |IntranetInRatio|写入带宽使用率|percent|instanceId|Average 、Minimum 、Maximum|
    |IntranetOut|读取网络带宽|Bytes/s|instanceId|Average 、Minimum 、Maximum|
    ...
    |UsedMemory|内存使用量|Bytes|instanceId|Average 、Minimum 、Maximum|
...
|Metric|描述|单位|Dimensions|Statistics|
|:-----|:-|:-|:---------|:---------|
|net\_tx.rate|流出带宽|bits/s|instanceId|Value|
|net\_rx.rate|流入带宽|bits/s|instanceId|Value|
...
|BPS|网络带宽峰值BPS，单位时间内网络流量的最大值|bits/s|instanceId|Average 、Minimum 、Maximum|
...
|InternetOut|下行流量|Bytes|instanceId|Average 、Minimum 、Maximum|
...
|net\_rx.rate|网络流入带宽|bits/s|instanceId|instanceId指带宽包ID|
|net\_tx.rate|网络流出带宽|bits/s|instanceId|instanceId指带宽包ID|
...
|Metric|监控项名称|单位|Dimensions|
|:-----|:----|:-|:---------|
|LogInflow|写入流量|Bytes|project 、logstore|
|NetFlow|原始数据大小|Bytes|project 、logstore|
|SumQPS|总体QPS|Count|project 、logstore|
|LogMethodQPS|操作次数|Count|project 、logstore|
|LogCodeQPS|服务状态|Count|project 、logstore|
|SuccessdByte|客户端解析成功流量|Bytes|project 、logstore|
|SuccessdLines|客户端解析成功行数|Lines|project 、logstore|
|FailedLines|客户端解析失败行数|Lines|project 、logstore|
...
|InflowLine|写入行数|Lines|project 、logstore|
|LogOutflow|读取流量|Bytes|project 、logstore|
...
|InternetInRate|网络流入带宽|Bytes/s|clusterId、serviceId 、instanceId|
|InternetOutRate|网络流出带宽|Bytes/s|clusterId、serviceId 、instanceId|
|MemoryAmount|内存使用量|Bytes|clusterId、serviceId 、instanceId|
|MemoryUtilization|内存使用率|%|clusterId、serviceId 、instanceId|
|IOReadRate|磁盘IO读速率|Bytes/s|clusterId、serviceId 、instanceId|
|IOWriteRate|磁盘IO写速率|Bytes/s|clusterId、serviceId 、instanceId|
|GPUMemoryUsed|GPU内存使用量|bytes|clusterId、instanceId、 gpuId|
...
|:-----|:----|:-|:---------|
|net\_rx.rate|网络流入带宽|bits/s|instanceId|
|net\_tx.rate|网络流出带宽|bits/s|instanceId|
|net\_rx.Pkgs|网络每秒流入数据包数|packages/s|instanceId|
|net\_tx.Pkgs|网络每秒流出数据包数|packages/s|instanceId|
...
|:-----|:----|:-|:---------|:-|
|net\_rx.rate|网络流入带宽|bits/s|instanceId|instanceId指共享带宽ID|
|net\_tx.rate|网络流出带宽|bits/s|instanceId|instanceId指共享带宽ID|
|net\_rx.Pkgs|网络流入数据包|packages/s|instanceId|instanceId指共享带宽ID|
|net\_tx.Pkgs|网络流出数据包|packages/s|instanceId|instanceId指共享带宽ID|
...
|InternetOutRateByConnectionArea|区域流出带宽|bits/s|cenId、geographicSpanId|
|InternetOutRateByConnectionRegion|地域流出带宽|bits/s|cenId、geographicSpanId、localRegionId、oppositeRegionId|
...
|VBRInternetOutRate|边缘路由器流出带宽|bits/s|cenId、vbrInstanceId|
|VBRInternetInRate|边缘路由器流入带宽|bits/s|cenId、vbrInstanceId|
...
|Metric|监控项含义|单位|Dimensions|
|:-----|:----|:-|:---------|
|instance\_internetin\_rate|实例维度网络上行带宽|bits/s|nodeId、instanceId|
|instance\_internetout\_rate|实例维度网络下行带宽|bits/s|nodeId、instanceId|
|node\_internetin\_rate|节点维度网络上行带宽|bits/s|nodeId|
|node\_internetout\_rate|节点维度网络下行带宽|bits/s|nodeId|
|user\_internetin\_rate|用户维度网络上行带宽|bits/s|空|
|user\_internetout\_rate|用户维度网络下行带宽|bits/s|空|

## 开放搜索 {#section_shg_kvz_cfb .section}

-   project为acs\_opensearch，采样周期见下表最小监控粒度。
-   Dimensions中的regionId字段的赋值，可通过[DescribeRegions](https://help.aliyun.com/document_detail/25609.html)接口查询。

|Metric|监控项含义|单位|Dimensions|最小监控粒度|
|:-----|:----|:-|:---------|:-----|
|DocSizebyApp|存储容量|Bytes|regionId、appName、appId|10分钟|
